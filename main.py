...
class Root(tk.Tk):
    def __init__(
        ...
        **kwargs
        ...
        self.title(
            "Six Key Braille Input: Unicode Braille"
            )
        self.config(
            bg="grey20"
            )
    ...
    def create_widgets(self):
        ...
        self.input_area = tk.Text(self.main_frame, height=25, width=48, bg="lavender", blockcursor=True, insertbackground="yellow", cursor = "cross red")
        self.input_area.grid(column=0, row=5, columnspan=2)
        self.input_area.config(font=("Calibri", 24), state="disabled")
        self.label2 = tk.Label(self.main_frame, text = ' ')
        self.label2.config(font=("Arial", 18, "bold"), bg="grey20", fg="snow")
    ...
    def key_combine(self, event):
        ...
        if lower_caps in letter_combination_mapping:
            ...
            self.display(output)
            curr = self.input_area.index(tkinter.INSERT)
        else:
            hold_key_list.clear()
        pass
